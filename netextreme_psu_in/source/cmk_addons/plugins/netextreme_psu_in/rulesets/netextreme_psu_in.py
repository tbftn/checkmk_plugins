...
# Email : alexander.vogel.2305@gmail.com
# Date  : 2025-08-29

# WATO for plugin netextreme_psu_in
        ...
        elements={
            ...
            'psu_powered_off': DictElement(
                parameter_form=ServiceState(
                    title=Title('State if PSU is "Powered Off"'),
                    help_text=Help('Default monitoring state if the PSU is present but has no power.'),
                    prefill=DefaultValue(2)
                )
            ),
            'psu_failed': DictElement(
                parameter_form=ServiceState(
                    title=Title('State if PSU is "Failed"'),
                    help_text=Help('Default monitoring state if the PSU and power are present, but an error has occurred.'),
                    prefill=DefaultValue(2)
                )
            ),
        }
